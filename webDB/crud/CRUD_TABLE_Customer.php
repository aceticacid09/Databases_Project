...
<?php 
    ...
    if(isset($_REQUEST['delete_id'])) {
        ...
        $delete_stmt = $db->prepare('DELETE FROM customer WHERE no = :o');
        $delete_stmt->bindParam(':o', $no);
        $delete_stmt->execute();

        header('Location:CRUD_TABLE_Customer.php');
    } 
?>
...
<!-- php (part showing data) -->
				<?php
					...
					<tr>
						...
						<td>$row[address]</td>
						<td>
							<a class='btn-edit' href='edit_Customer_crud.php?update_id=$row[no]'>Edit</a>
						</td>
						<td>
...
<?php 
    ...
    if(isset($_REQUEST['btn_insert'])) {
        ...
        if (empty($idcard)) {
            ...
            try {
                if (!isset($errorMsg)) {
                    $insert_stmt = $db->prepare("INSERT INTO customer(idcard,name,phone,address) VALUES (:n, :a, :p, :x)");
                    $insert_stmt -> bindParam(':n', $idcard);
                    $insert_stmt -> bindParam(':a', $name);
                    $insert_stmt -> bindParam(':p', $phone);
                    $insert_stmt -> bindParam(':x', $address);
